...
@implementation ISGroupDetailVC
...
- (void)viewDidLoad {
    ...
    [refreshControl addTarget:self action:@selector(getAllRequestOfGroup:) forControlEvents:UIControlEventValueChanged];
    [self.tblGroupDetailList addSubview:refreshControl];
...
- (NSInteger)numberOfSectionsInTableView:(UITableView *)tableView {
    if(self.groupRequestArray.count == 0) {
        ...
        {
            noDataLabel.text = @"No Requests in group. Please use \"+\" to add new requests";
        }
...
NSComparisonResult dateSort(NSString *s1, void *context) {
...
- (IBAction)btnArchivesClicked:(id)sender {
    ISArchivedRequestVC *groupDetailVC = [self.storyboard instantiateViewControllerWithIdentifier:@"ISArchivedRequestVC"];
